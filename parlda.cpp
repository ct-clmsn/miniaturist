...
inline void par_gibbs(CompressedMatrix<double> const& dwcm, DynamicMatrix<double> & tdcm, DynamicMatrix<double> & twcm, std::vector<std::size_t> & tokens, DynamicVector<double> & ztot, DynamicVector<double> & probs, const std::size_t n_topics, const double N, const double alpha, const double beta) {
...
void par_train_lda(const std::vector<std::size_t> & thread_idx,
    ...
    for(std::size_t i = 0; i < iterations; ++i) {
        ...
        //std::for_each(std::begin(thread_idx), std::end(thread_idx), [&tokens, &dwcm, &tdcm, &twcm, &twcm_base, &ztot, &probs, n_topics, alpha, beta](const std::size_t i) {
            par_gibbs(dwcm[i], tdcm[i], twcm[i], tokens[i], ztot[i], probs[i], n_topics, N, alpha, beta);
            twcm[i] -= twcm_base;
        });
